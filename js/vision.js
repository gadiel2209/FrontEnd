...
async function cargarVision() {
    try {
        ...
        const texto = data.vision?.descripcion || 'Contenido no disponible.'
        const id = data.vision?.id

        document.getElementById('textoVision').textContent = texto

        if (esAdmin) {
            document.getElementById('seccionAdmin').style.display = 'block'
            document.querySelector('#formVision textarea').value = texto
            document.querySelector('#formVision textarea').dataset.id = id
        }
    } catch (err) {
        document.getElementById('textoVision').textContent = 'No se pudo cargar la visión.'
        console.error(err)
    }
}

async function guardarVision(e) {
    e.preventDefault()
    const textarea = document.querySelector('#formVision textarea')
    ...
    try {
        const res = await fetch(`${API_BASE}/vision/${id}`, {
            method: 'PUT',
